  ...
  try {
    ...
    if (!startDate || !endDate || !documentType) {
      return NextResponse.json(
        { message: 'Missing required parameters: startDate, endDate, and documentType are required' },
        { status: 400 }
      );
    }

    // Find the corresponding process type for the document type
    const dataTypeConfig = DataTypeOptions.find(dt => dt.value === documentType);
    if (!dataTypeConfig) {
      return NextResponse.json(
    ...
    if (error.response) {
      ...
      // Try to extract more detailed error message from XML if available
      if (error.response.data && typeof error.response.data === 'string') {
        const errorMatch = error.response.data.match(/<code>(.*?)<\/code>.*?<message>(.*?)<\/message>/s);
        if (errorMatch && errorMatch.length >= 3) {
          message = `ENTSO-E Error ${errorMatch[1]}: ${errorMatch[2]}`;
        }
      }
    } else if (error.request) {
      message = 'No response received from ENTSO-E API, please try again later';
    }
